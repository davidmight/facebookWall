...
void IBE_Decryptor :: extract(ECn& A,ZZn& x,ZZn& y)
{ 
    x=(A.get_point())->X;
    y=(A.get_point())->Y;
}

void IBE_Decryptor :: extract(ECn& A,ZZn& x,ZZn& y,ZZn& z)
{ 
    big t;
    x=(A.get_point())->X;
    y=(A.get_point())->Y;
...
ZZn2 IBE_Decryptor :: line(ECn& A,ECn& C,ZZn& slope,ZZn2& Qx,ZZn2& Qy)
{ 
    ZZn2 n=Qx,w=Qy;
    ZZn x,y,z,t;
#ifdef AFFINE
...
ZZn2 IBE_Decryptor :: vertical(ECn& A,ZZn2& Qx)
{
    ZZn2 n=Qx;
    ZZn x,y,z;
#ifdef AFFINE
...
void IBE_Decryptor :: g(ECn& A,ECn& B,ZZn2& Qx,ZZn2& Qy,ZZn2& num)
{
    ZZn  lam,mQy;
    ZZn2 d,u;
    big ptr;
...
BOOL IBE_Decryptor :: fast_tate_pairing(ECn& P,ZZn2& Qx,ZZn2& Qy,Big& q,ZZn2& res)
{ 
    int i,nb;
    Big n,p;
    ECn A;
...
BOOL IBE_Decryptor :: ecap(ECn& P,ECn& Q,Big& order,ZZn2& cube,ZZn2& res)
{
     ZZn2 Qx,Qy;
     Big xx,yy;
#ifdef SCOTT
...
int IBE_Decryptor :: H2(ZZn2 x,char *s)
...
Big IBE_Decryptor :: H3(char *x1,char *x2)
{
    sha256 sh;
    char h[HASH_LEN];
    Big a;
...
void IBE_Decryptor :: H4(char *x,char *y)
{ // hashes y=h(x)
    int i;
    sha256 sh;
    shs256_init(&sh);
...
void IBE_Decryptor :: strip(char *name)
...
void IBE_Decryptor :: decrypt(char *result, char *message, char *key)
{
